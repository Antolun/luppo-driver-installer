"""
Pisi Package Manager Backend Interface.
...
from models.gpu_device import DriverOption, DriverStatus
from i18n import tr

# Try importing native pisi module if available
NATIVE_PISI_AVAILABLE = False
try:
    import pisi.db.installdb
    import pisi.db.packagedb
    NATIVE_PISI_AVAILABLE = True
except ImportError:
    NATIVE_PISI_AVAILABLE = False


class PisiBackend:
    """Interface to Pisi Package Manager."""
    ...
    def __init__(self, demo_mode: bool = False):
        ...
        if NATIVE_PISI_AVAILABLE:
            try:
                self._installdb = pisi.db.installdb.InstallDB()
                self._packagedb = pisi.db.packagedb.PackageDB()
            except Exception:
                pass
    ...
    def _check_installed_cli(self, pkg_name: str) -> Tuple[bool, Optional[str]]:
        try:
            res = subprocess.run(["pisi", "info", pkg_name], capture_output=True, text=True, timeout=5)
    ...
    def install_package(self, pkg_name: str, extra_pkgs: list = None) -> Generator[Dict[str, Any], None, bool]:
        ...
        yield {
            "percent": 5,
            "message": tr("pisi_install_start", pkg_name=pkg_name),
            "log": tr("pisi_cmd_install", cmd=cmd_str)
        }

        if self.demo_mode:
            # Simulated installation steps
            steps = [
                (15, tr("pisi_verifying_repos"), 0.5),
                (30, tr("pisi_downloading", pkg=pkg_name, size="48.2 MB"), 1.0),
                (55, tr("pisi_unpacking"), 0.8),
                (75, tr("pisi_copying_files"), 1.2),
                (90, tr("pisi_updating_xorg"), 0.7),
                (100, tr("pisi_install_done", pkg=pkg_name), 0.3)
            ]
        ...
        exec_cmd = ["pkexec", "pisi", "install", "--yes"] + pkgs_to_install
        ...
        try:
            ...
            if return_code == 0:
                yield {
                    "percent": 100,
                    "message": tr("pisi_install_comp_msg"),
                    "log": tr("pisi_install_comp_log")
                }
                return True
            else:
                yield {
                    "percent": 0,
                    "message": tr("pisi_err_exit_code", code=return_code),
                    "log": tr("pisi_err_install_fail", code=return_code)
                }
                return False
            ...
            yield {
                "percent": 0,
                "message": tr("pisi_err_exit_code", code=str(e)),
                "log": tr("pisi_exception", msg=str(e))
            }
            return False
    ...
    def remove_package(self, pkg_name: str) -> Generator[Dict[str, Any], None, bool]:
        ...
        yield {
            "percent": 5,
            "message": tr("pisi_remove_start", pkg=pkg_name),
            "log": tr("pisi_cmd_remove", pkg=pkg_name)
        }

        if self.demo_mode:
            steps = [
                (30, tr("pisi_checking_deps"), 0.5),
                (60, tr("pisi_deleting_files"), 0.8),
                (90, tr("pisi_refreshing_mods"), 0.5),
                (100, tr("pisi_remove_done", pkg=pkg_name), 0.3)
            ]
            for pct, msg, sleep_t in steps:
                time.sleep(sleep_t)
                yield {"percent": pct, "message": msg, "log": msg}
        ...
        exec_cmd = ["pkexec", "pisi", "remove", "--yes", pkg_name]
        try:
            process = subprocess.Popen(
                exec_cmd,
                stdout=subprocess.PIPE,
            ...
            while True:
                ...
                if line:
                    ...
                    yield {"percent": progress, "message": clean_line[:80], "log": f"[PISI] {clean_line}"}

            return_code = process.poll()
            if return_code == 0:
                yield {
                    "percent": 100,
                    "message": tr("pisi_remove_comp_msg"),
                    "log": tr("pisi_remove_comp_log")
                }
                return True
            else:
                yield {
                    "percent": 0,
                    "message": tr("pisi_err_exit_code", code=return_code),
                    "log": tr("pisi_err_remove_fail", code=return_code)
                }
                return False
        except Exception as e:
            yield {
    ...
    def update_repositories(self) -> Generator[Dict[str, Any], None, bool]:
        """Update PiSi package repositories."""
        yield {
            "percent": 10,
            "message": tr("pisi_update_repos_start"),
            "log": tr("pisi_cmd_update_repo")
        }
        ...
        if self.demo_mode:
            ...
            yield {
                "percent": 50,
                "message": tr("pisi_downloading_repos"),
                "log": tr("pisi_updating_repos_log")
            }
            time.sleep(1.0)
            yield {
                "percent": 100,
                "message": tr("pisi_repos_updated_msg"),
                "log": tr("pisi_repos_updated_log")
            }
            return True

        exec_cmd = ["pkexec", "pisi", "update-repo"]
        try:
            process = subprocess.Popen(exec_cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
            for line in process.stdout:
                yield {"percent": 50, "message": line.strip()[:80], "log": f"[PISI] {line.strip()}"}
            process.wait()
