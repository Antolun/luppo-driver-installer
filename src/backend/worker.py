"""
PyQt6 Worker threads for asynchronous hardware scanning and Luppo package manager tasks.
"""
...
from src.backend.luppo_backend import LuppoBackend
...
class PackageCheckWorker(QThread):
    """Worker thread to query Luppo package status for driver options."""
    ...
    def __init__(self, gpus: List[GPUDevice], demo_mode: bool = False, parent=None):
        super().__init__(parent)
        self.gpus = gpus
        self.luppo = LuppoBackend(demo_mode=demo_mode)

    def run(self):
        self.log_message.emit(tr("worker_luppo_querying"))
        for gpu in self.gpus:
            for option in gpu.driver_options:
                self.luppo.check_package_status(option)
                self.log_message.emit(tr("worker_luppo_status", pkg=option.package_name, status=option.status.value, version=option.installed_version or tr("status_not_installed")))
...
class DriverActionWorker(QThread):
    ...
    def run(self):
        ...
        if self.action_type == "install":
            gen = self.luppo.install_package(self.pkg_name, self.extra_pkgs)
            try:
                while True:
                    update = next(gen)
            ...
            gen = self.luppo.remove_package(self.pkg_name)
            try:
                while True:
                    update = next(gen)
                    self.progress.emit(update["percent"], update["message"], update["log"])
            ...
            gen = self.luppo.update_repositories()
            try:
                while True:
                    update = next(gen)
                    self.progress.emit(update["percent"], update["message"], update["log"])
