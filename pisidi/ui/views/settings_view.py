...
from pisidi.ui.icon_helper import get_icon, create_icon_label
...
class SettingsView(QWidget):
    ...
    def _init_ui(self):
        ...
        # 2. PiSi Package Repository Management
        pisi_card = QFrame()
        pisi_card.setProperty("class", "GPUCard")
        pc_layout = QVBoxLayout(pisi_card)
        pc_layout.setSpacing(6)

        pc_header = QHBoxLayout()
        pc_header.setSpacing(6)
        pc_header.addWidget(create_icon_label("package.svg", 16))
        lbl_pc_title = QLabel(tr("pisi_mgmt_title"))
        ...
        lbl_pc_desc = QLabel(tr("pisi_mgmt_desc"))
        ...
        btn_clean_cache.clicked.connect(self._clean_pisi_cache)
    ...
    def _clean_pisi_cache(self):
        HardwareDetector._run_cmd(["pisi", "dc"])
        QMessageBox.information(self, tr("msg_pisi_cache_title"), tr("msg_pisi_cache_text"))
